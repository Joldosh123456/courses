...
function CourseDetailsReviews() {
      ...
      reviewsState.map((elem, index) => (
        ...
        >
          ...
          <div className={`flex flex-col gap-2`}>
            ...
            <small className="text-xs">
              {elem.date
                ...
                .map((elem2: any, index2: number) =>
                  index2 == 1 ? months[Number(elem2) - 1] : elem2
                )
                .join(" ")}
            </small>
            ...
            <div
              className={`${css["CourseDetailsReviews__review-buttons"]} mt-2 flex items-center gap-2 font-bold`}
            >
              <Button
                className="!rounded-full"
  ...
  const reviewsRating = Number((reviewsState.reduce((sum, elem) => sum+=elem.rated,0)/reviewsState.length).toFixed(1))

  const diagramFunc = (rating: number) => {
    return reviewsState.filter(elem => elem.rated === rating).length/reviewsState.length*100
  }


  return (
    <article className={`${css["CourseDetailsReviews"]}`}>
      <div className="container flex gap-16">
        <div className={`${css["CourseDetailsReviews__content"]}`}>
          <header className="flex items-center justify-between static">
            <h1 className="text-2xl sm:text-4xl font-bold">Reviews</h1>
          ...
          <div
            className={`${css["CourseDetailsReviews__reviews-container"]} flex flex-col`}
          >
            {renderReviews}
          </div>
        </div>
        ...
        <aside
          className={`${css["CourseDetailsReviews__aside"]} h-fit p-8 rounded-2xl flex flex-col gap-6`}
        >
          {currCourse ? (
            <>
              <div
              ...
              >
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <p>{currCourse.reviews} Reviews</p>
                </div>
              </div>
              ...
              <div>
                {
                Array(5).fill(0).map((elem, index) => (
                  <div className={`${css["CourseDetailsReviews__aside-statistic"]} flex items-center justify-between gap-4`}>
                    ...
                    <div className="grow rounded-sm">
                      <div className="w-full h-full rounded-sm" style={{width: diagramFunc(Math.abs(index-5))+'%'}}></div>
                    </div>

                    <p>{reviewsState.filter((review, index2) => review.rated == (Math.abs(index-5))).length}</p>
                  </div>
                ) )               
                }
              </div>
