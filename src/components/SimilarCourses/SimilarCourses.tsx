...
import css from "./SimilarCourses.module.scss";
...
function SimilarCourses() {
  ...
  useEffect(() => {
    setCurrCourse(coursesData.filter((elem: course) => elem.id === Number(courseId))[0])
  }, [coursesData, location]);

  useEffect(() => {
    if (currCourse) {
      setCoursesArray(coursesData.filter(elem => elem.id !== currCourse.id && elem.category === currCourse.category).slice(0,3));
    }
  }, [currCourse, location])
  ...
  const renderCoursesCards = useMemo(
    ...
    [coursesArray, i18n.language, location]
  );

  return (
    <article
      className={`${css["SimilarCourses"]} ${
        darkScheme ? css["SimilarCourses-dark"] : ""
    ...
    >
      ...
      >
        <div
          className={`${css["SimilarCourses__header"]} w-full flex items-end justify-between`}
        >
          <div className="w-full sm:w-2/3 flex flex-col gap-6 pb-2 text-center sm:text-left">
            <h1 className="text-3xl sm:text-5xl font-bold">
              {t("SimilarCourses.title")}
            </h1>
          </div>

          <div
            className={`${css["SimilarCourses__swiper-buttons"]} hidden flex gap-1 h-fit [&>button]:p-2 [&>button]:rounded-full [&>button]:absolute [&>button]:sm:static`}
          >
            <button ref={prevBtn}>
        ...
        <div
          className={`${css["SimilarCourses__swiper"]} w-full flex flex-col sm:flex-row gap-10`}
        >
          {renderCoursesCards}
        </div>
